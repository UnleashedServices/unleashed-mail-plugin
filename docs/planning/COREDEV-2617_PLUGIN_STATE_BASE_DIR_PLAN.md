...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2126`) and its matrix change (`:2132-2136`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2159-2166`) stand as written;
...
via `additionalContext` (§7 row `:2502`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2225`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2502`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2126`) requires that `paths.sh`'s absence change *who computes* the
...
| 116 | **open the TRANSIENT name before testing what is there** | a FIFO at the transient name REFUSES; it does not hang. Measured: `set -C; : > fifo` never returns in either shell (`rc=124` at a 5s timeout), so an implementation relying on `set -C` alone stops every hook at source time |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2502` amended.
