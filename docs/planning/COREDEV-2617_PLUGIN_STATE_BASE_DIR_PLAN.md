...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2165`) and its matrix change (`:2171-2175`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2198-2205`) stand as written;
...
via `additionalContext` (§7 row `:2542`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2264`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2542`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2165`) requires that `paths.sh`'s absence change *who computes* the
...
| 82 | **use `mkdir -m 700 -p` instead of one-at-a-time creation** | ST-2 creates each missing ancestor SEPARATELY and E4 authenticates each immediately after creating it, so a failure at `unleashed-mail` leaves `.claude` present and `unleashed-mail` ABSENT. Fixture: both absent, and `.claude` fails authentication after creation (it inherits a refusing ACL). Correct: `.claude` exists, `unleashed-mail` does NOT, publisher reports `failed`. Under `-p` BOTH exist, because one call created them before anything was authenticated — the observable difference is the INTERMEDIATE, not the mode, which is why asserting the reader's or the publisher's tuple alone discriminates nothing |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2542` amended.
