...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2217`) and its matrix change (`:2223-2188`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2250-2218`) stand as written;
...
via `additionalContext` (§7 row `:2600`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2316`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2600`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2217`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2600` amended.
...
Measured by executing THIS TEXT in both shells, identical results: layer 1 yields
`[group:staff] [inherited] [allow] [list,add_file,file_inherit]` — 4 fields, so the verb is found at
position 3 on an inherited ACE and position 2 on a plain one, which is why ACL-2 locates it by TOKEN;
layer 2 yields `[list] [add_file] [file_inherit]` — 3 rights.
