...
**ST-7 — Atomic publication.** **Precondition, measured: if the path at `base.<key>` exists and is NOT a regular non-symlink file, the publisher does not attempt repair — it removes its transient, writes nothing, and reports `failed`.** `mv -f` onto a DIRECTORY moves the transient *inside* it (executed: the transient appeared as `base.KEY/tmpfile`), and onto a symlink-to-directory Darwin `mv` can follow the link and write outside the store; Darwin `/bin/mv` has no GNU `-T` (executed: `illegal option -- T`). Such a shape arises only from same-uid interference or a crash, so refusal is correct and recovery is an operator `rm` — the same disposition as a stale entry. A publish writes the transient in the SAME directory as the entry and then renames it onto `base.<key>` with `mv -f`. Every command of the publish has both stdout and stderr redirected to `/dev/null`; their exit statuses are still examined, and the write decision is taken from them. Two publishers holding the same base value converge on one entry name and require no serialisation: the bytes are identical and the rename is atomic, so either order leaves the same content, and a reader never observes a torn or momentarily-absent entry as a result of any publication.
...
**ENT-2 — Exactly one line, and exactly its bytes.** The entry authenticates only if **its size in bytes equals the length of the line read plus one** (the single trailing newline), AND — **in zsh only** — the line contains no NUL. One check, two clauses, and together they are total over every malformed shape: a second line, a missing trailing newline, trailing bytes after the newline, and an embedded NUL.
