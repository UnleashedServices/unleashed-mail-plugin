...
| 82 | **use `mkdir -m 700 -p` to satisfy the exact-0700 store rule** | a pre-existing `0755` store yields sentinel, `OK=0`, `SOURCE=unresolved`, `POINTER_STATE=stale`, one diagnostic — rule −1 fires before any entry is examined, so this holds even for an EMPTY store, and it is never chmod'ed |
