...
> | restore consumed it | no `work-context-snapshot-*` remains, per the `rm -f "$SNAP"` at `sessionstart-restore.sh:99` |
>
...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2363`) and its matrix change (`:2369-2373`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2396-2403`) stand as written;
...
via `additionalContext` (§7 row `:2748`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2462`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2748`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2363`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2748` amended.
