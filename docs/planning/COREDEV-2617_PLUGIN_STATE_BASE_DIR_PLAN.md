...
via `additionalContext` (§7 row `:2548`), so a pointer naming attacker-writable storage is a
...
This **amends §7's consumer row** (`:2548`), which currently requires both snapshot scripts to leave
...
| 42 | **accumulate distinct targets as a space-delimited string** | a store whose base values are `/a b` and `/a` and `/b` — an ORDERED COLLISION, not isolated odd characters — is counted as THREE distinct bases; under the mutation the delimiter makes `/a b` indistinguishable from the pair and the count collapses to two, so the conflict is missed. The glob case is `/g*` beside `/gx`, asserted with `LC_ALL=C`, `set -f` OFF, and every expansion quoted, since an unquoted accumulator would glob-match the sibling. *(Round 104, codex: values containing a space and a glob char IN ISOLATION are each counted once by the mutated accumulator too, so the old fixture discriminated nothing.)* |
...
| 105 | **let the own-entry clause assign a state directly** | a `0644` own entry yields exactly ONE enum value AND the value is the one the ordered exits assign. **The fixture must force DIVERGENCE**: a state where the clause's direct assignment would be `current` while the ordered post-scan exit assigns `stale` (own entry present but failing authentication). Observing only the final value cannot discriminate — a direct assignment that the ordered exit later overwrites, or that happens to agree, leaves exactly one final value either way |
...
| 130 | **drop ENT-2's read-status clause, keeping only the byte count and the zsh NUL test** | an entry holding `/private/tmp` followed by a NUL and NO newline yields sentinel, `OK=0`, `SOURCE=unresolved`, `POINTER_STATE=stale`, one diagnostic. Under the mutation bash AUTHENTICATES it — measured `rc=1`, `size=13`, `${#line}+1=13` — while zsh still refuses, so a zsh-only cell cannot discriminate it and the oracle is the BASH arm |
| 131 | **carve out the publisher when `/usr/bin/id -un` fails while the ACL enumerator EXISTS** | the publisher REFUSES: `POINTER_STATE=failed`, one diagnostic, no entry written. Under the mutation it skips the ACL clauses and publishes into a store it could not evaluate — the fail-open AUTH-1(h) forbids for a present-but-failed probe. Row 129 mutates an untyped PRINCIPAL, not a failed probe, and cannot discriminate this |
| 132 | **exceed BUD-1's 50 ms reader budget** | a READER resolution on the reference machine with a two-entry store completes within 50 ms; the harness MEASURES it and a run outside the limit fails verification. Without this row the wall-clock gate is stated and never proved |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2548` amended.
...
state. The name is resolved ONCE per resolution, not once per component. **If `/usr/bin/id -un` fails, exits non-zero, or prints anything but a single non-empty line, every component of this resolution REFUSES — publisher and reader alike, with NO carve-out.** AUTH-1 clause (h) grants the publisher's exemption only where NO enumerator exists, and explicitly withholds it from a present probe that failed; extending it here would let a publisher skip the ACL clauses whenever the identity probe fails while the enumerator is present and working — the fail-open AUTH-1 forbids by name. *(Round 104, codex: the round-102 wording said "the same publisher carve-out", which contradicted AUTH-1(h) and ACL-4 outright.)* Without that, a failed probe left the predicate undefined on the one input every ACE comparison depends on.
...
governs that count — this cap is the reader's and is not a bound on the publish path.** A **READER** resolution stays under 50 ms wall-clock on the reference machine with
