...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2518`) and its matrix change (`:2524-2528`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2551-2558`) stand as written;
...
via `additionalContext` (§7 row `:2903`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2617`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2903`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2518`) requires that `paths.sh`'s absence change *who computes* the
...
| 179 | **enumerate the store without forcing globbing on, and without restoring the caller's setting** | a caller with globbing DISABLED — bash `set -f`, zsh `setopt noglob`, or `SHELLOPTS=noglob` inherited from the environment (bash imports it at startup) — reads a store holding one valid entry: the specification resolves (`1 pointer none`, no diagnostic); under the mutation (the forcing removed — the zsh arm back to `no_nomatch` alone, which is NOT globbing, and the bash save/clear/restore gone) the pattern reaches the loop LITERAL, rule 0 disposes of it as vanished, and the healthy store reports `0 unresolved none` with the "no plugin-state entry is present" notice SS-1 stays silent on. Both shells, plus the bash `SHELLOPTS` cell. Three cells hold in BOTH builds so the fix costs nothing: the caller's `noglob` is still in force after the read — asserted FUNCTIONALLY, since `$-` reports zsh's option as `F` and a flag-letter test reads as restored where nothing was — an EMPTY store still reports `none`, and a caller who left globbing ON keeps it and still resolves. *(PR #67, codex pass 17 — reproduced.)* |
| 180 | **refuse a plugin-data base that does not exist yet — or create it WITHOUT authenticating its parent first, or WITHOUT `umask 077`** | `CLAUDE_PLUGIN_DATA` names a directory nothing has written to — a FIRST SESSION, the state in which this library's own writers create it with `mkdir -p` moments later: the specification authenticates the parent, creates the directory at 0700 and publishes (`created`, stderr SILENT), the next process reports `current`, and a reader with no variable resolves it; under the mutation (E2a's creation block removed) EVERY run reports `failed` with a publication-failure line and the store is never seeded. TWO FURTHER MUTANTS, one per bound, because the first implementation shipped without both and this row measured them: with the `umask 077` subshell dropped a fresh install at `umask 002` creates the base `0775`, its own chain then refuses it and the publish reports `failed` on that hook and every later one, where the specification creates `0700` and publishes; with the `mkdir` moved back above the parent authentication an other-writable parent gets the directory CREATED and only then refused — the state is `failed` in both builds, so the FILESYSTEM is the discriminator. Both shells. Four refusal controls must still hold on the specification: a FILE, a SYMLINK to a directory, a DANGLING symlink (whose target is NOT created) and an other-writable parent each report `failed` with one diagnostic and no store. The `-L` half of the creation guard is stated, not claimed: `/bin/mkdir -p` on a dangling symlink creates nothing in either build, so it does not discriminate at the outcome level. *(PR #67, codex pass 17 — reproduced; re-pinned after the follow-up this row's own two findings forced.)*  A fourth mutation restores the ORDER that shipped before pass 22 — `mkdir -p` ahead of the fold — and is discriminated by the filesystem, not the state word: both builds report `failed` for a value with `..` ahead of a symlink, and only the mutant leaves directories behind. |
| 181 | **derive the entry key from the caller's SPELLING of the base rather than from a lexical fold of it — or fold it PHYSICALLY, through `cd -P`/`pwd -P`** | four spellings of ONE directory — `<d>/sub`, `<d>/./sub`, `<d>/x/../sub`, `<d>//sub` — published by four fresh processes: the specification folds the value lexically in shell (drop `''` and `.`, pop on `..`, rebuild; no fork, no `cd`), so the store holds ONE entry, the runs report `created current current current`, and a reader answers `1 pointer <d>/sub` — IDENTICALLY IN BOTH SHELLS; under the mutation (the fold removed) the store holds FOUR entries, three runs report `conflict`, and every later reader is permanently `0 unresolved conflict`. The PHYSICAL fold is the second mutation and it is pinned as an ABSENCE (`_pb_phys` must not reappear): measured, `cd -P`/`pwd -P` returns the caller's spelling in bash and the ON-DISK case in zsh, so one mis-cased value published by a bash hook and a zsh shell left TWO entries and a permanent `conflict` — a regression the fix introduced and this row caught, whose two-shell cell now asserts ONE entry as a regression guard. ENC-4's case-folding cost is asserted as ACCEPTED in both shells (`SUB` beside `sub` = two entries, `conflict`), and one control covers the residual the fold declares: a value whose parent is a SYMLINK is refused by PCH-1's walk (`failed`) in both builds. The unenterable-directory cell of the `cd -P` version is REMOVED, not weakened: a lexical fold enters nothing, both builds report `created` for a `0600` base, and a cell whose two builds agree proves nothing. *(PR #67, codex pass 17 — reproduced; re-pinned after the follow-up this row's own finding forced.)* |
...
| 186 | **neutralise `noglob` for the store scan but leave `failglob` — a SEPARATE option — in force, or clear it and never restore it** | an authenticated but EMPTY store, which is the state of every first session, read by a caller with `shopt -s failglob`: the specification answers `0 unresolved none` with the one "no plugin-state entry is present" notice; under the mutation (the `failglob` save/clear and its restore removed, `noglob` untouched) bash aborts the whole command list on the unmatched `<store>/base.*` BEFORE rule 0 can dispose of it, so `_unleashed_scan_store` never returns — measured through the `paths.sh` route a hook actually runs, all four protocol variables are left UNSET, no diagnostic of ours is emitted, bash's own `no match:` line names the store, and under `set -e` the sourcing shell EXITS (rc≠0, empty stdout). The oracle spells the UNSET case out: `"$_UNLEASHED_BASE_OK"` renders unset and empty identically, so an oracle built on the ordinary tuple would read the kill as `"  "` and could not tell it from a resolver that answered with empty strings. A SECOND mutation removes the RESTORE alone — the whole-guard mutant never clears the option and so cannot see it — and a bash caller then LOSES `failglob` across an ordinary read. bash only: `setopt failglob` answers `no such option` in zsh (asserted), so the zsh arm is asserted EQUAL in both builds. Three cells hold in BOTH builds: the caller's `failglob` survives the read (asserted FUNCTIONALLY, in a subshell, because the option's whole effect is to kill the shell that expands the pattern — and the probe is shown able to answer GLOB-OK, so it discriminates), a caller who never set it does not acquire it, and a HEALTHY store still resolves `1 pointer none` under `failglob`. *(PR #67, codex pass 23 — reproduced.)* |
| 187 | **let the encoder's upper-case `case` arms be evaluated under an inherited `nocasematch`, or clear the option and never restore it** | a caller with `shopt -s nocasematch` — ordinary, and inherited because these libraries are SOURCED: the specification's walk is byte-exact (`/a` → `_sa`, `/A` → `_s_ca`, `/café` → `_scaf_xc3_xa9`, identical to zsh and to bash with the option off); under the mutation (the guard and its restore removed) `/a` and `/A` BOTH encode to `_s_ca` and `/café` to `_s_cc_ca_cf_xc3_xa9` — every lower-case letter having taken an upper-case arm — so ENC-1 injectivity is gone. **THE ORACLE IS THE KEYS, never survival**: every cell here survives in both builds, so a survival-only check passes a build in which one directory has two names. End to end: both builds publish `created` with zero diagnostics and one entry, and only the mutant's store then reads `0 unresolved stale` with one diagnostic from an ordinary hook — publisher and reader disagreeing, silently and permanently. A SECOND mutation removes the RESTORE alone: the keys are then right and the CALLER's `nocasematch` is left switched off, asserted with a FUNCTIONAL probe (a `case` that can only match with the option on), and a caller who never set it must not acquire it in any build. zsh has no `shopt` at all (asserted), so its arm is asserted EQUAL; the honest control is bash with the option OFF, where every build agrees on the canonical keys. *(PR #67, codex pass 23 — reproduced.)* |
| 188 | **assign `LC_ALL=C` without first detecting that the caller made it READONLY — or detect it with a fork, accept `C.UTF-8`, or "restore" an accepted readonly by unsetting it** | four mutations against ENC-3's guard, at BOTH sites (the encoder's walk and the reader's byte count). (a) The unguarded assignment: with `readonly LC_ALL=C` the specification derives `_stmp_sabc`, leaves `LC_ALL` at `C` and is SILENT, while the mutation destroys bash's whole command list — the `||` arm does not run either, so a caller cannot even detect the failure — and kills the zsh shell outright; at the reader site the same mutation leaves the tuple UNSET in bash and kills zsh on a store the specification resolves `1 pointer none`. (b) `C.UTF-8` back on the accepted list: the specification REFUSES (no key, shell alive, both shells) and the mutation encodes `/café` as `_scaf_xc3` in bash and `_scaf_xe9` in zsh against the correct `_scaf_xc3_xa9` — one directory, three keys — **so the oracle is the KEYS**; at the READER site that same mutation is asserted NON-discriminating and equal, because the encoder's own refusal at ENT-3 already fail-closes the read, and the row says so rather than implying a cell it does not have. (c) Restore-by-unsetting the accepted readonly `C`: fatal in zsh (no output at all) and a stray shell error on bash's stderr where the specification is silent — the third restore state is what this measures. (d) The FORKING probe my first version used: under `ulimit -u 1` the specification still derives the exact key in both shells and the mutation derives NOTHING in bash, which is ENC-2 re-measured (row 045's obligation, at the new site). The honest control throughout is a WRITABLE `LC_ALL`, where every build agrees, and the row asserts the primitives it rests on rather than assuming them: an assignment to a readonly is fatal in both shells even for its OWN value, bash's `unset -v` fails non-fatally on one, and zsh's `unset` does not. Both shells. *(PR #67, codex pass 23 — reproduced.)* |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2903` amended.
...
**(c) Fail-closed on a strange environment — visibly.** Where the store, an entry, an ancestor or a target is in any state these rules do not describe, the resolution is refused, nothing is written, nothing is repaired and nothing is deleted. Refusal is the correct direction and is not negotiable. **But fail-closed is only correct when the failure is VISIBLE, and a silent refusal is a defect of the same weight as a wrong resolution.** SS-1 is the mechanism: its six-value partition carries `conflict`, `stale` and `failed` to the user on every SessionStart source. One defect of exactly this shape is now CLOSED and one is still open. An inherited `noglob` made a healthy store read as empty and take RD-8 rule 4, whose `none` SS-1 is specified to stay silent on — RD-9 now forces the glob for the scan and restores the caller's flag *(row 179)*. An inherited `failglob` was worse than silent: on an empty store bash aborted the scan outright, so the resolver set NO protocol variable at all and there was nothing for SS-1 to report — RD-9 now neutralises that option beside `noglob` *(row 186)*. An inherited `nocasematch` was silent in the other direction: the store resolved to `stale` forever because the publisher and every reader disagreed about one directory's key — ENC-1 now forces the walk case-sensitive *(row 187)*. A store bricked by a dangling entry still has no recovery a user can act on: it fails closed, and it fails closed INVISIBLY, which this clause forbids.
...
`BASH_ENV` names a file that bash sources at the start of every non-interactive shell, **before the first line of any script or library runs** — `ZDOTDIR` gives zsh the same. A parent that can set it has arbitrary code execution in our shell before `paths.sh` is read, and can therefore satisfy any in-process guard we could write, including a genuine `declare -r` stamp with no pid prediction *(G2/G4, reproduced; both closed as residuals rather than fixed, correctly)*. The same premise reaches the guard's own PRIMITIVES, and the instance check is no exception: `declare -p` and `typeset -p` are subject to function lookup, so a `set -a` parent exporting a fake `declare` that prints a readonly declaration FORGES the stamp that check reads, and the exec'd child then reports `base=/attacker ok=1` *(codex, PR #67 pass 21 — reproduced end to end; the same class as the exported `.`, `[` and `unset` shadows refuted earlier on that PR)*. Naming those primitives one at a time — `builtin declare`, `builtin typeset` — would close instances while the class stayed open, and would read as though it were shut; the honest statement is this one. No guard placed in a file that is sourced can precede code that runs before sourcing begins.
...
| `set -euf` / `setopt noglob` in a caller | the store scan glob stays literal; healthy store reads as empty; SS-1 silent *(F17/G8, FIXED — RD-9 forces the glob and restores the caller's flag; row 179)* |
| `shopt -s failglob` in a caller, on an EMPTY store | a SEPARATE option from `noglob`, and fatal rather than wrong: bash aborts the command list on the unmatched `base.*`, the scan never returns, all four protocol variables are left UNSET with no diagnostic of ours, and under a sourcing script's `set -e` the shell exits *(codex, PR #67 pass 23 — reproduced; FIXED — RD-9 saves, clears and restores `failglob` beside `noglob`; row 186)* |
| `shopt -s nocasematch` in a caller | the encoder's upper-case `case` arms match lower-case bytes, so `/a` and `/A` both encode to `_s_ca`: the publisher authenticates its OWN wrong key and reports `created`, and every ordinary hook then reports `stale`, silently and permanently *(codex, PR #67 pass 23 — reproduced; FIXED — ENC-1 forces the walk case-sensitive and restores the caller's option; row 187)* |
| `readonly LC_ALL=…` in a caller or a wrapper | ENC-3's `LC_ALL=C` is a FATAL assignment, not a scoped one: the shell died in BOTH shells before a single protocol variable existed, and no shell-level guard survives it *(codex, PR #67 pass 23 — reproduced; FIXED — the attribute is DETECTED fork-free, a readonly `C`/`POSIX` proceeds and is left entirely alone, anything else refuses; row 188)* |
| A first session, with the plugin-data directory not yet created | every hook printed a publication failure and the store was never seeded *(FIXED — PUB-9 E2a creates it; row 180)* |
| A `umask` of `002` when E2a creates the base | the base is created group-writable, its own chain then refuses it, and every later hook reports `failed` *(FIXED — E2a creates under `umask 077` in a subshell; row 180 cell (a))* |
| An other-writable parent, with E2a creating the base | the base directory is created before the chain is authenticated — a write outside the store by the refusal path *(FIXED — E2a authenticates the PARENT before it creates anything; row 180 cell (b))* |
| A value with `..` ahead of an existing SYMLINK, on a first install | E2a's `mkdir -p` ran BEFORE E2b's fold, so the components were created through the link and OUTSIDE the authenticated chain, and only then refused — `<h>/new/../link/created` left `<h>/new` and `<d>/outside/created` on disk while the publish reported `failed` *(codex, PR #67 pass 22 — reproduced; FIXED: the fold is lexical and needs nothing to exist, so it now runs FIRST and E2a authenticates the nearest existing ancestor OF THE FOLDED PATH — here the symlink itself, which ST-4 refuses — leaving nothing on disk; row 180)* |
| The same mis-cased `CLAUDE_PLUGIN_DATA` in a bash hook and a zsh shell | `cd -P` kept the spelling in bash and returned the on-disk case in zsh: two keys, two entries, permanent `conflict` *(FIXED — E2b folds LEXICALLY, so both shells derive one key; row 181's two-shell cell is the regression guard)* |
| A symlinked component POPPED by a following `..` in the value | the lexical fold names a directory the kernel would not: the store advertises one directory while this process's writers use another *(OPEN — narrower than what E2b fixes; stated in E2b)* |
| `set -e` in a sourcing script | `readonly` re-assignment and a bare status capture each exited the sourcing shell *(T60, T30)* |
| Two hooks publishing the same base | 27 of 64 concurrent publishers reported `stale`; user-facing failure diagnostic *(REDUCED, not closed — PUB-9 E7b's single rescan takes 11-38 of 64 down to 0-3; row 182)* |
| An absent `HOME` under zsh | zsh substitutes the passwd `HOME`; PUB-9 E1 unreachable; a store written under a home the environment never named *(STATED RESIDUAL — see below; row 183)* |
| A case-insensitive volume, a `..` or `//` segment | two entries for one directory; permanent `conflict` *(the `.`/`..`/`//` carriers are FIXED by PUB-9 E2b — row 181; ENC-4's case-folding carrier stands)* |
...
**STATED RESIDUAL — PUB-9 E1 cannot detect an ABSENT `HOME` under zsh, and the opt-out is the protection.** zsh initialises its special `HOME` parameter from the PASSWD DATABASE before any sourced line runs: `env -i /bin/zsh` reports `HOME=/Users/<u>` where `env -i /bin/bash` reports it unset (measured, both shells; an in-shell `unset HOME` leaves it unset in both, so the substitution is a STARTUP property). E1's refusal — "HOME is empty or not absolute" — is therefore **unreachable for an absent `HOME` under zsh**, and a wrapper or harness that clears `HOME` specifically to prevent persistent writes is protected in bash and NOT in zsh, where publication proceeds into the store under the passwd home *(codex, PR #67 pass 17 — reproduced)*. This is recorded as a residual rather than fixed because **no post-startup test can distinguish "zsh filled `HOME` in from passwd" from "the caller set `HOME` to exactly that value"**, and a guard that cannot tell those apart either refuses a legitimate resolution or accepts the case it exists to catch. E1 remains reachable in BOTH shells for a `HOME` that is set and EMPTY, which is why §7 step 3f(i) and HAR-1 spell the unresolved cell `HOME=""` and never `unset HOME`. **The portable protection is the explicit opt-out this library already carries — `_UNLEASHED_PUBLISH_OK=0` (PUB-9 E0), which suppresses publication in both shells and composes no `${HOME}`-rooted path at all** — and every harness in this repo that must not write uses it. Row 183 pins all three: the arm divergence as a measured fact, E1's reachability for an empty `HOME`, and the opt-out as the load-bearing mechanism, mutating E0 rather than the predicate because the predicate has no discriminating mutant to build.
...
**ENC-1 — Entry-key encoder (Invariant P).** K is a pure function of a byte string. K(v) is computed by a SINGLE left-to-right walk over the bytes of v with `LC_ALL=C` in force, emitting for each input byte exactly one of four disjoint markers and otherwise that byte unchanged: `_` (0x5F) -> `_u`; `/` (0x2F) -> `_s`; an upper-case ASCII letter C (0x41-0x5A) -> `_c` followed by the lower-case form of C; a byte >= 0x80 or < 0x20 -> `_x` followed by exactly two lower-case hexadecimal digits giving that byte's value. There are exactly FOUR markers: no fifth marker, no case folding, no locale dependence and no other transformation. The four rows above are the whole table, so 0x7F (DEL) is emitted unchanged. Decoding is unambiguous — `_` is always followed by exactly one of `u`, `s`, `c`, `x`; `c` consumes one further output byte and `x` consumes two — therefore K is injective. The entry key for a base value is K applied to the absolute base path the publisher holds. No other statement of the encoder anywhere in the document is normative. **THE `case` THAT PERFORMS THE WALK IS EVALUATED CASE-SENSITIVELY, WHATEVER THE CALLER LEFT SET.** The lower-casing of an upper-case letter is performed by one `case` arm per letter (ENC-2), and bash's `nocasematch` — an ordinary option a caller or a wrapper may have enabled, and one this library inherits because it is SOURCED rather than executed — makes those arms match the lower-case bytes too: measured, with it in force `/a` and `/A` BOTH encode to `_s_ca`, and `/café` encodes to `_s_cc_ca_cf_xc3_xa9`, every lower-case letter having taken an upper-case arm, so K is no longer injective and ENC-5's whole consequence chain is void *(codex, PR #67 pass 23 — reproduced)*. The derivation therefore records whether the option is set, clears it for the walk, and restores it on every path by which the derivation ends — exactly what ENC-3 requires of `LC_ALL`, and the restore is a separate obligation from the clearing, because a guard that clears and never restores leaves the CALLER's `case` statements case-sensitive for the rest of the process. The guard is bash-only: zsh has no such option for `case` (measured: `shopt` is not a zsh builtin there). The failure is SILENT and permanent, not loud — the publisher derives the wrong key, authenticates its OWN wrong key and reports `created` with zero diagnostics, and every ordinary hook then computes the canonical key, finds the ENT-3 name/content pair inconsistent and reports `stale` for as long as the entry survives. **The obligation is therefore on the KEYS, not on survival**: every case survives this defect, so an oracle that checks only that nothing died passes a build in which one directory has two names *(row 187)*.

**ENC-2 — Fork-free single-pass derivation.** The derivation of K forks zero times and runs no external program: it is a plain assignment built from parameter expansion, with the lower-casing of an upper-case ASCII letter performed by a `case` arm per letter. Forbidden: command substitution (`k="$(...)"`), `tr`, `sed`, any other external command, bash-4's `${x,,}` (the shell floor is bash 3.2.57), the two-pass `${v//…}` substitution form, and any `%`-based escaping. The walk is a single pass — each input byte is consumed exactly once and its output appended, and output bytes are never re-examined or re-encoded. Consequently no substitution order exists: the `case` arms may appear in any order and there is no "escape the escape character first" obligation. **THIS BINDS ENC-3'S READONLY PROBE TOO.** The attribute test ENC-3 requires before it touches `LC_ALL` is part of the derivation, so it must also fork zero times: `$( declare -p LC_ALL )` and `$( typeset -p LC_ALL )` are command substitutions and are forbidden here for exactly the reason `k="$(...)"` is. This is not a hypothetical — the first version of that guard used `$( declare -p LC_ALL )`, and measured under `ulimit -u 1` the encoder then produced NO key at all where the shipped derivation still produces the exact key *(row 045, which caught it; re-measured as row 188 cell (3))*. The two permitted probes are builtins: under zsh `${(t)LC_ALL}`, which reports `scalar-readonly-special` for a readonly and `scalar-special` otherwise, written inside a single-quoted `eval` because bash 3.2 cannot PARSE that expansion at all; under bash `unset -v LC_ALL`, which on a readonly fails NON-fatally there (measured) while an assignment would kill the shell.

**ENC-3 — LC_ALL scope and restoration.** Before deriving K the resolver records whether `LC_ALL` is set in the caller's environment and, if set, its value. It sets `LC_ALL=C` for the duration of the derivation only and, immediately afterwards, restores exactly that state: re-assigning the saved value if `LC_ALL` was set on entry, and executing `unset LC_ALL` if it was unset on entry (an empty `LC_ALL` is not the same as an absent one). The restoration runs on every path by which the derivation ends, so a consumer's locale is unchanged after the resolver runs. **A CALLER MAY HAVE MADE `LC_ALL` READONLY, AND THEN THAT ASSIGNMENT IS FATAL — NOT SCOPED, NOT RECOVERABLE.** Measured in BOTH shells: `readonly LC_ALL=C.UTF-8` followed by sourcing killed the shell outright, before a single protocol variable was established, so a hook DIED where it was specified to resolve; the same is true of an assignment of the variable's OWN value. No shell-level guard survives it — not `if ! LC_ALL=C; then …`, not `{ LC_ALL=C; } || true`, not a redirection of its stderr; only a SUBSHELL survives, and a subshell cannot set the locale for the walk that follows *(codex, PR #67 pass 23 — reproduced)*. **The attribute is therefore DETECTED before any assignment is attempted, never assigned to speculatively**, using the fork-free per-shell probes ENC-2 names. The detection has exactly THREE outcomes and each is normative: (i) not readonly — `LC_ALL=C` is assigned and the entry state restored exactly as above; (ii) readonly and exactly `C` or `POSIX` — the walk is byte-oriented already, so the derivation PROCEEDS and the variable is left ENTIRELY ALONE on exit, neither reassigned nor unset. That is a THIRD restore state, distinct from "was unset on entry, so unset it again", because `unset` of a readonly is FATAL in zsh (measured) and a restore-by-unsetting would kill the shell one line after the bug it was written to prevent; (iii) readonly and anything else — the derivation REFUSES, returning non-zero and deriving no key, because the byte semantics ENC-1 requires cannot be established. **`C.UTF-8` IS NOT AN ACCEPTED VALUE.** It is a UTF-8 locale, so the walk is CHARACTER-wise under it: measured, a readonly `C.UTF-8` encoded `/café` as `_scaf_xc3` in bash and `_scaf_xe9` in zsh against the correct `_scaf_xc3_xa9` — one directory, three keys, and exactly the per-shell divergence T04 exists to prevent. It WAS listed as acceptable in the first version of this guard and every case "survived"; the KEYS said otherwise, which is why the obligation here is stated on the keys and not on survival. **THE SAME THREE-OUTCOME GUARD GOVERNS THE READER'S BYTE COUNT.** `_unleashed_auth_entry` takes `${#line}` under `LC_ALL=C` for precisely the reason the encoder does, so it carries the identical detection: under an accepted readonly `C` it resolves normally, and under a readonly value it cannot accept it refuses the ENTRY — which surfaces as `stale` through rule 1, with its one diagnostic, rather than as a dead shell *(row 188)*.
...
**RD-9 — Store enumeration.** The reader enumerates exactly the glob `<store>/base.*`, from inside the scanning function. `.pub.<pid>.<uniq>.<key>` temporaries lie outside that glob by construction, are never enumerated, and a crash-orphaned temporary therefore changes no resolution. Under zsh the scanning function executes `setopt local_options no_nomatch glob` (the `glob` half is the paragraph below), guarded by `[ -n "${ZSH_VERSION:-}" ]`, because `setopt` is a zsh builtin and bash's `command not found` under the `set -euo pipefail` these libraries are sourced with aborts the sourcing shell in exactly the arm the guard protects; zsh restores the option at function return. Under bash the unmatched pattern remains literal and is disposed of by rule 0's test — the enumeration itself carries no existence guard. The empty-store case is asserted under zsh independently for each of the five family files; one passing arm is not evidence about the other four. **GLOBBING IS FORCED ON FOR THE SCAN, AND THE CALLER'S SETTING IS RESTORED BEFORE THE FUNCTION RETURNS.** The store is enumerated by a pattern, so the enumeration is correct only where pathname expansion is enabled, and a caller who disabled it — bash `set -f`, zsh `setopt noglob`, the defensive `set -euf` idiom in a wrapper, or an inherited `SHELLOPTS=noglob`, which bash imports at startup — left `<store>/base.*` unexpanded: the literal reached the loop, rule 0 disposed of it as vanished, and a HEALTHY store holding one valid entry reported `OK=0 SOURCE=unresolved POINTER_STATE=none` with the "no plugin-state entry is present" notice, in BOTH shells *(codex, PR #67 pass 17 — reproduced; row 179)*. The scanning function therefore enables globbing for its own body: under zsh by adding `glob` to the `setopt local_options` line already there — **`no_noglob` is NOT an option name in zsh**, and setting it is an error that leaves globbing off (measured: `setopt: no such option: no_noglob`, which is why the first attempt at this fix moved the bash arm only) — and under bash by saving `case $- in *f*)`, clearing the flag, and restoring it before the single `return`, because bash has no `local_options` scope. **The restoration is asserted FUNCTIONALLY**, by expanding a pattern after the scan and requiring it to stay literal: `$-` reports zsh's `noglob` as `F` and not `f`, so a flag-letter test reads as restored where nothing was restored. An EMPTY store still takes rule 4 under a caller's `noglob`, in both shells — forcing the glob must never turn an unmatched pattern into a phantom entry. **`failglob` IS A SEPARATE bash OPTION FROM `noglob`, AND ON AN EMPTY STORE IT IS FATAL RATHER THAN MERELY WRONG.** Turning `noglob` off does not turn `failglob` off, and with `failglob` set bash ABORTS the command list on the unmatched `<store>/base.*` — before the loop body runs, so rule 0's vanished-entry test never gets the chance to dispose of it. Measured through the `paths.sh` route a hook actually runs: on an authenticated but EMPTY store the scanning function never returned, `unleashed_resolve_base` never completed, and `_UNLEASHED_BASE_OK`, `_UNLEASHED_BASE_SOURCE`, `_UNLEASHED_POINTER_STATE` and `_UNLEASHED_BASE_RESOLVED` were ALL LEFT UNSET with no diagnostic of ours — a consumer calling `unleashed_base_ok` then reads an unset variable, and under a sourcing script's `set -e` the shell EXITED *(codex, PR #67 pass 23 — reproduced)*. The scanning function therefore saves and clears `failglob` beside `noglob` in the bash arm, with `shopt -q failglob` / `shopt -u failglob`, and restores it before the single `return`; the restore is a separate obligation from the clearing, and is pinned by its own mutation, because a scan that clears the caller's `failglob` and never puts it back has moved the defect from the store to the caller. zsh has NO such option — measured, `setopt failglob` answers `no such option: failglob` — so the zsh arm is unchanged and is asserted equal in both builds rather than claimed to differ. The empty store is the ORDINARY case this clause protects: it is the state of every first session *(row 186)*.
...
**PUB-9 — Ordered publish exit mapping.** Every exit of the publish routine maps to exactly one `_UNLEASHED_POINTER_STATE` value. The exits are evaluated in this order, first match wins: (E0) `_UNLEASHED_PUBLISH_OK` is `0` — the publish is skipped entirely, nothing is composed or opened, no scan → `none`; (E1) `_UNLEASHED_HOME_OK` is false — no `${HOME}`-rooted path is composed or opened at all, no scan → `failed`; (E2) this process's own base value is NOT PUBLISHABLE — it fails a clause of TGT-1, or its TARGET CHAIN fails to authenticate (PUB-7), neither of which this exit enumerates, for the reason PUB-7 gives one clause above — so no key is derived, nothing is composed or opened under the store, and no scan runs → `failed`; **(E2a) a base that does not exist YET is CREATED, not refused — after its PARENT authenticates, and at 0700.** `CLAUDE_PLUGIN_DATA` names the directory the HOST will use, and on a first session nothing has written there yet — while this library's own writers (`marker_write`, `log_append`, `context_*`) create it lazily with `mkdir -p` moments later. Refusing it meant every hook of a first session printed a publication-failure line and the store was NEVER seeded, on an ordinary healthy machine with no adversary *(measured: `failed` with one diagnostic on every run, and the very next writer created the directory; codex, PR #67 pass 17 — reproduced; row 180)*. The creation is attempted when — and only when — NOTHING is present at that path, tested `[ ! -e ] && [ ! -L ]`, **both parts required** so a dangling symlink is never created through. **Two clauses bound it, and both are normative because the first implementation shipped without them and row 180 measured what each cost.** (i) The base's PARENT must authenticate under PCH-1 BEFORE anything is created; a `mkdir -p` that ran first left a directory under an other-writable parent and only then refused — a write outside the store performed by the refusal path, which is exactly what E4 step (i) forbids on the store chain *(measured: `failed` with the directory created; after: `failed` with nothing on disk)*. (ii) The creation runs under `umask 077` in a subshell, so every component `-p` makes is `0700` whatever the caller's `umask` is; inheriting the ambient `umask` produced a `0775` base at `umask 002`, which the base's OWN chain walk then refused — the publish reported `failed` on that hook and on every later one, because the group-writable directory was left behind *(measured: `created` at 0700 under `umask 002` after the fix, `failed` at 0775 before it)*. A creation that does not succeed still fails E2: the existing-directory test below is unchanged, and a FILE, a SYMLINK (dangling or not) or a directory whose chain does not authenticate each still report `failed` with one diagnostic and leave no store. **(E2b) the published value is the caller's value folded LEXICALLY — never a value obtained from the filesystem.** Before the key is derived the publisher walks the value's `/`-separated segments in shell, dropping empty segments and `.`, popping the last kept segment on `..`, and rebuilding; it forks nothing and consults no directory. The key is an injective encoding of the VALUE (ENC-5), so two spellings of ONE directory produced two entries and every later reader was permanently `conflict`: publishing `<d>/sub`, `<d>/./sub` and `<d>/x/../sub` left THREE entries and a reader at `OK=0 … conflict`, a state only a manual delete clears *(codex, PR #67 pass 17 — reproduced; row 181)*. **THE FOLD IS LEXICAL BECAUSE A PHYSICAL ONE IS NOT PORTABLE ACROSS THE TWO TARGET SHELLS.** The first implementation used `CDPATH='' cd -P -- "$v" && pwd -P`; measured, on a case-insensitive volume bash returns the spelling it was asked for and zsh returns the ON-DISK case, so ONE mis-cased `CLAUDE_PLUGIN_DATA` published by a bash hook and a zsh shell derived two keys, left two entries and produced a permanent `conflict` that did not exist before the fix — a regression introduced by the fix and found by row 181, which now carries the two-shell cell as a regression guard. ENC-8 requires one value to produce byte-identical keys in both shells; a normalisation that asks the filesystem what a path is called cannot satisfy it. **THE FOLDED VALUE FACES E2's CONSTRAINTS AGAIN, before anything is written.** They were applied to the caller's SPELLING, and the fold can produce a shape they already rejected: `/.` and `/Users/..` both fold to `/`, which is absolute and carries no trailing segment but IS a trailing slash. The publisher then derived the key of `/`, wrote `base._s` holding `/`, and its OWN post-scan refused that entry by TGT-1's trailing-slash clause — so the publish reported `failed` HAVING LEFT THE ENTRY BEHIND, and every later reader was `stale` until someone deleted it by hand *(measured; codex, PR #67 pass 20 — reproduced; row 184)*. A poison entry is worse than a refusal, so the folded value is re-tested for absoluteness, for a trailing slash, and for being the root itself, and a failure there is E2: one diagnostic, nothing composed under the store, an EMPTY store. **The state word alone does not distinguish the two behaviours — both report `failed` — so the obligation is on the STORE and on what a later reader sees.** A `..` that folds to a real directory is unaffected and still publishes. **What E2b does NOT close, stated rather than assumed:** case, exactly as ENC-4 declares — `SUB` beside `sub` still publishes a second entry and still conflicts, now identically in both shells; and symlinked components, which the fold does not resolve. The latter is bounded but not empty: a value whose parent is a symlink is REFUSED by PCH-1's chain walk (measured, `failed` in both shells), but a value in which a symlinked component is POPPED by a following `..` — `<h>/lnk/../x`, where `<h>/lnk` names a directory elsewhere — folds to a path the kernel would not have produced, so the store can advertise `<h>/x` while this process's own writers land in `<h>/deep/x` (measured, both shells, both directories existing; if the folded path does not exist the publish fails closed at the existing-directory test). That shape is OPEN, it is the two-directory defect this ticket exists to remove, and it is narrower than what the fold fixes — it is recorded here rather than traded for the shell divergence the physical version cost. The value this process USES in memory stays the caller's spelling; only what is PUBLISHED is folded, which is what the key and ENT-3 need; (E3) the `NAME_MAX` budget for `.pub.<pid>.<uniq>.<key>` cannot be satisfied — either it is exceeded, or `/usr/bin/getconf` is absent, exits nonzero, or emits anything that is not a decimal number, which FAILS CLOSED and is not treated as an unlimited budget — nothing written → `failed`; (E4) a missing ancestor cannot be created, or `bases/` or an ancestor of it fails ST-3 or ST-4 — whether it pre-existed or this publisher has just created it, since a freshly created store can inherit a default ACL that fails ST-3 → `failed`. **E4 evaluates the store chain in THIS ORDER, and no `mkdir` precedes the validation of what already exists.** (i) Authenticate every EXISTING component of PCH-1's walk from `/` downward — ACL clauses included — BEFORE creating anything; a failure here creates NOTHING, which is what ACL-6 already requires of every refusal path. (ii) Create each missing component in turn (ST-2), authenticating each immediately after creating it — **and authenticate, with the same no-follow chain predicate, any component that is PRESENT at this step but was ABSENT at (i)** (it appeared in between; `-d` follows a symlink, so without this a same-uid process that planted `.claude` as a symlink to an outside directory after (i) had the next component's `mkdir` run THROUGH the link — a directory created outside the store by the refusal path, before (iii) noticed; PR #67, codex pass 13 — reproduced, both shells; row 171). (iii) Authenticate the completed chain. **Validation of the existing prefix before the first `mkdir` is the load-bearing step**: with `HOME=/safe/home` a symlink to `/victim` and `.claude` absent, an implementation that creates first and validates afterwards runs `mkdir "$HOME/.claude"` THROUGH the symlink, leaves `/victim/.claude` behind, and only then reports `failed` — a write outside the store performed by the refusal path itself. *(Round 90, codex: E4 required the complete-chain verdict only before an ENTRY was written, and explicitly permitted created ancestors to remain, so the order above was permitted but not required and two conforming implementations differed on whether `/victim/.claude` appears.)* *(Round 88, codex: ST-4 and rule −1 bounded the store chain at `${HOME}` while AUTH-1 requires `/` downward, so a `/Users` carrying a foreign mutating `allow` ACE passed E2 and E4, the publisher wrote its entry, and the post-scan COMPLETE authentication then rejected that same ACE and reported `stale` — with ST-8 forbidding removal. A durable entry every reader refuses forever, created by a publisher obeying the rules: the identical shape round 74 fixed for the TARGET chain, surviving on the STORE chain because its lower bound was written in a different rule.)* **NO ENTRY and no transient is written; ancestors this publisher already created are LEFT IN PLACE.** ST-2 creates them one at a time, so a failure at `unleashed-mail` after `.claude` succeeded, or a store rejected after creation for an inherited default ACL, necessarily leaves the earlier directories — and ST-3/ST-4 forbid repairing or removing them. Those directories are `0700` and euid-owned, so leaving them is harmless and the next run reuses them; claiming "nothing written" was simply false, and no implementation could have satisfied it. *(Round 84, codex.)* **This check omits the ACL clauses under EXACTLY the condition AUTH-1 clause (h) states and no other: the platform has NO ENUMERATOR AT ALL.** A present enumerator that exits non-zero, prints nothing, or prints an unparseable ACE line REFUSES here, as everywhere else, because it may be failing precisely because the component is hostile. **And the carve-out extends to the STORE and its ancestors, not only to the publisher's own entry file** — stated because AUTH-1(h) is worded in terms of "its own entry" and this exit authenticates a directory: without the extension an enumerator-less runner fails at E4 and never reaches PUB-7, so the exemption both rules rely on would be dead code. *(Round 74, codex: rounds 70 and 72 narrowed the word in ACL-4 and then in AUTH-1(h) and left it wide HERE, so following E4 accepted a component that ACL-4 refuses — the third round in which this one exemption was fixed in some of its sites and not all.)* (E5) no unique temporary name can be obtained within TMP-1's three TOTAL attempts — a REFUSED exclusive create whose name is then PRESENT is a lost race and ONE consumed attempt, so three of them are this exit (mutant row 154) — nothing written → `failed`; (E6) the temporary's creation or write, or the `mv`, fails for any reason — INCLUDING a refused exclusive create whose name is ABSENT afterwards (`ENOSPC`, `EROFS`, `EACCES`: a CREATION failure, never a consumed attempt — mutant row 155; P-4 states how the primitive tells the two refusals apart), INCLUDING ST-7's refusal to repair a `base.<key>` that exists and is not a regular non-symlink file, **and including a transient that was CREATED SUCCESSFULLY but whose mode read back is not exactly 0600** (P-4: a POSIX default ACL supplies permissions `umask` does not mask, so this is reachable on a store ACL-3 accepts). In that case the publisher ATTEMPTS to remove its transient (best-effort, ST-7), writes no entry, and reports `failed` — a failed removal does not change the state, the diagnostic count, or anything a reader sees → `failed`. *(Round 84, codex: P-4 stated the readback obligation and no rule in §4.2a-S carried the resulting exit, so enum totality was not DERIVED — an implementer had to infer from §4.2a-P prose which exit it belonged to, which is precisely what §4.2a-S's completeness claim denies.)*; (E7) otherwise the write-or-skip has completed, and the publisher scans the store and applies the ORDERED post-scan exits below. E1 through E6 are stated in execution order and all map to `failed`, so their relative order does not affect the mapping; E0 outranks E1, and E7 is reachable only when no earlier exit matched. *(Round 61: E2 was missing. PUB-7 states the TGT-1 precondition and gives it the state `failed`, but this list — which calls itself exhaustive — did not carry it, so the one exit that writes nothing and derives no key was the one an implementer building from the list would not have.)* The post-scan exits are themselves ORDERED, first match wins: (P1) this process's own entry is missing → `failed`, whether or not this process wrote it; (P2) else any remaining entry, this process's own included, fails authentication → `stale`; (P3) else two or more entries authenticate → `conflict`; (P4) else `created` if this process wrote the entry, `current` if it skipped the write. The scan applies the same ordered reader rules the reader applies, including the vanished-entry skip: an entry enumerated but gone when opened and NOT a symlink is skipped and is not an entry, so it is never treated as malformed; a `base.*` symlink, dangling or not, is a failing entry. A publisher does not refuse — it has already resolved — it reports what it saw. **E7b — ONE RESCAN BEFORE A REPAIR STATE IS REPORTED.** When the first post-scan would take P1 or P2 — this process's own entry missing or unusable, or any surviving entry failing — the publisher scans the store ONCE MORE and re-evaluates the ordered post-scan exits against the second scan's counts. Two ORDINARY hooks publishing the SAME base into a fresh store race each other: the second `mv` replaces the entry between the scan's stat and its open, ENT-2b's inode binding correctly rejects the object that was opened, and the publisher reported `stale` — or `failed` for its own entry — although the surviving entry is valid and BOTH processes agree on its content *(measured on a fresh store, 8 trials of 8 concurrent publishers released together: 11-38 of 64 reported a repair state; codex, PR #67 pass 17 — reproduced; row 182)*. A rescan of a SETTLED store authenticates while a genuinely unusable entry fails both times, so the retry converts the transient case ONLY: a corrupt sibling entry still reports `stale`, and two publishers of DIFFERENT bases still report `conflict`. **EXACTLY ONE retry, never a loop** — under sustained concurrent publication a loop spins, and the honest report for a store that will not settle is the repair state. **The retry REDUCES the transient case and does not eliminate it: with it, 0-3 of the same 64 publishers still report a repair state** (measured), because a publisher can lose the race a second time inside the rescan; a rule claiming zero would claim more than the code achieves, and the mutant row asserts the bound rather than the zero.
