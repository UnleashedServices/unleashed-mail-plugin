...
root; the consumer enumeration is now in the implementation order. Rounds 1-17 are in §10-§26.
**Ticket:** `COREDEV-2617` (Epic `COREDEV-2485`) · **High** — a live defect, reproduced on this machine
**Last Updated:** 2026-07-31 (round 17, post-gate revision — helper guarded; consumer detection specified)
...
3. Implement the chosen resolution; add N2 with the **unset** case.
   ...
   **When `paths.sh` is absent** the helper's resolver is simply not defined, and the consumer's own
   inline fallback applies — the identical posture the three libs already take. The bridge never aborts.
...
5. **Enumerate and guard every consumer** before quarantine, each with its **required control flow** —
   ...
   >
   > **Consumers branch on `_UNLEASHED_BASE_OK`** — `1` resolved, `0` unresolved — the same flag the
