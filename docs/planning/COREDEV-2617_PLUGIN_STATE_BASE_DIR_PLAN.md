...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2404`) and its matrix change (`:2410-2414`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2437-2444`) stand as written;
...
via `additionalContext` (§7 row `:2789`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2503`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2789`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2404`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2789` amended.
