...
1. `CLAUDE_PLUGIN_DATA` non-empty → **use it, `OK=1`, source `host-env`. Unconditionally — this branch
   ...
   **this publisher's own entry** `${HOME}/.claude/unleashed-mail/bases/base.<key>`, where `<key>` is the
   ...
   repaired rather
   than reported `current` and then refused by step 2. **(Superseded on one point by PUB-9: a publish that
FAILS may already have composed and opened `${HOME}`-rooted paths — E3 through E6 all follow the
`NAME_MAX` probe and the ancestor creation. Only E0 and E1 compose nothing.)** If `_UNLEASHED_HOME_OK`
...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2155`) and its matrix change (`:2161-2165`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2188-2195`) stand as written;
...
via `additionalContext` (§7 row `:2532`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2254`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2532`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2155`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2532` amended.
