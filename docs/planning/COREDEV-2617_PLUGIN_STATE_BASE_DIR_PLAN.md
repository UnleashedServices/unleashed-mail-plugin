...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2413`) and its matrix change (`:2419-2423`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2446-2453`) stand as written;
...
via `additionalContext` (§7 row `:2798`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2512`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2798`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2413`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2798` amended.
