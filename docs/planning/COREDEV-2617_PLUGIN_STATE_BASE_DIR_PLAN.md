...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2420`) and its matrix change (`:2426-2430`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2453-2460`) stand as written;
...
via `additionalContext` (§7 row `:2805`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2519`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2805`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2420`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2805` amended.
