...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2130`) and its matrix change (`:2136-2140`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2163-2170`) stand as written;
...
via `additionalContext` (§7 row `:2506`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2229`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2506`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2130`) requires that `paths.sh`'s absence change *who computes* the
...
| 116 | **open the TRANSIENT name before testing what is there** | the publisher does not HANG. Measured: `set -C; : > fifo` never returns in either shell (`rc=124` at a 5s timeout). The oracle is the absence of the hang, NOT a `failed` state: P-6 permits three retries with a fresh `$RANDOM`, so a correct implementation may skip a FIFO'd candidate and publish successfully on the next name — an oracle demanding `failed` would fail a correct implementation |
| 122 | **ignore UNNAMED default ACL entries on Linux** | a `0700` euid-owned target carrying `default:group::rwx` REFUSES; under the mutation it authenticates and every consumer-created child beneath it inherits group-write, because Linux seeds a new object's access ACL from the parent's default ACL rather than from `umask`. Row 104 covers a NAMED default user and cannot discriminate this. Linux-only; gated on the CI probe |
...
| 121 | **apply the ACL clauses at PUB-9 E4's store-and-ancestor check on an enumerator-less platform** | same fixture, same oracle, different exit — under the mutation the publisher fails at E4 instead. AUTH-1(h) enumerates three carve-out sites and rows 106, 120 and 121 pin one each |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2506` amended.
...
**BUD-1 — Source-time budget.** The complete resolution — including its ACL evaluation, which is `/usr/bin/uname -s` followed by `/bin/ls -lde` on Darwin or `/usr/bin/getfacl -pc` on Linux per component of BOTH the entry chain and the target chain — runs at most once per PROCESS. **On the PUBLISH path the budget is stated per WALK, not per component, because the publisher necessarily walks the same chains more than once and the repeats are not redundant.** The target chain is walked before the write (PUB-7's precondition) and again in the post-scan re-verification; the STORE chain is walked at E4 before publication and again in the post-scan authentication of the publisher's own entry; and on the no-write `current` path the write-or-skip predicate authenticates the entry a further time. Each pair is separated by the write, or by another process's possible write, so **a verdict cached from before it is not evidence about the state after it** — which is why these are not collapsed. The budget is therefore: a READER performs ONE walk of each chain. A PUBLISHER performs at most TWO walks of the target chain and TWO of the store chain, **and up to THREE own-entry authentications on the no-write `current` path** — the pre-write target-chain check, the write-or-skip decision, and the post-scan re-verification. *(Round 80, agy and codex concordantly: the previous "one per decision point" capped at two what the `current` path necessarily performs three times, so no conforming implementation could satisfy it. This is the second time this budget has been stated in a form nothing could meet; it is now derived by counting the decision points the rules themselves create, not asserted.)* *(Round 78, codex: the once-per-component wording, and then the round-76 wording that counted only the target chain, were both unachievable — a correct implementation could not satisfy either.)* The five sourced libraries share its result and no consumer re-runs it, and within that one resolution a READER probes no component of either chain more than once, and a PUBLISHER probes the target chain at most twice, the store chain at most twice, and its own entry at most three times on the `current` path. The budget bounds how many times the walk runs, never whether it runs: a publisher runs the complete follower predicate with the ACL clauses included, so a hook pays the ACL cost too. A resolution on EITHER path — publisher/hook and reader alike — stays under 50 ms wall-clock on the reference machine with a two-entry store. The bound is measured rather than asserted, printed beside `pwd` and `git rev-parse HEAD`; a run outside it fails verification.
...
then VERIFY.** Measured on Darwin: yields `600` in **both** shells. **The umask is not sufficient on
its own.** A POSIX DEFAULT ACL on the containing directory supplies permissions that `umask` does not
mask, so on Linux a store carrying `default:group::r--` — which ACL-3 ACCEPTS, because the mask
conjunct is not satisfied — can yield a transient at other than 0600, and `mv` preserves that mode
onto the entry. The publisher therefore READS the transient's mode back through the single P-2
accessor immediately after creating it and before writing into it, and refuses (`failed`, transient
removed) if it is not exactly 0600. It does NOT `chmod`: ST-5 forbids the create-then-chmod window,
and a mode that arrives wrong is evidence about the directory, not something to paper over. This is
UNMEASURED on Linux — the CI probe of §6 must run it under a default ACL before the Linux arm is
treated as proved. The subshell scopes the `umask` change so it cannot leak to a
