...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2157`) and its matrix change (`:2163-2167`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2190-2197`) stand as written;
...
via `additionalContext` (§7 row `:2534`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2256`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2534`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2157`) requires that `paths.sh`'s absence change *who computes* the
...
| 82 | **use `mkdir -m 700 -p` to satisfy the exact-0700 store rule** | the PUBLISHER reports `SOURCE=host-env`, `OK=1`, `POINTER_STATE=failed` and one diagnostic, and NO entry is written, when it meets a pre-existing `0755` store — asserting the READER's `stale` cannot discriminate this, because rule −1 refuses that store whether or not the publisher wrongly wrote into it |
...
| 104 | **ignore `default:` ACL entries (Linux)** | a target carrying `default:user:other:rwx` yields sentinel, `OK=0`, `SOURCE=unresolved`, `POINTER_STATE=stale`, one diagnostic — the `.state` it would create must not be attacker-writable |
...
| 107 | **skip the ACL clauses for a FOREIGN entry where no enumerator exists** | a READER on an unenumerable platform yields sentinel, `OK=0`, `SOURCE=unresolved`, `POINTER_STATE=stale`, one diagnostic, for a foreign entry, with no carve-out |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2534` amended.
