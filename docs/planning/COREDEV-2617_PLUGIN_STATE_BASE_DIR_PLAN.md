...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2418`) and its matrix change (`:2424-2428`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2451-2458`) stand as written;
...
via `additionalContext` (§7 row `:2803`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2517`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2803`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2418`) requires that `paths.sh`'s absence change *who computes* the
...
| 167 | **guard the `paths.sh` definition block on ONE function instead of the complete resolver API** | bash: `export -f unleashed_resolve_base` in a parent, then a child sources `paths.sh`: the specification defines the whole API (`unleashed_plugin_base` prints the sentinel or the base, `unleashed_base_ok` is defined); under the mutation the block is skipped and `unleashed_plugin_base` is `command not found`. bash only (zsh cannot export functions). *(PR #67, codex pass 11.)* |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2803` amended.
