...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2168`) and its matrix change (`:2174-2178`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2201-2208`) stand as written;
...
via `additionalContext` (§7 row `:2545`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2267`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2545`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2168`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2545` amended.
...
state. The name is resolved ONCE per resolution, not once per component. **If `/usr/bin/id -un` fails, exits non-zero, or prints anything but a single non-empty line, the ACL condition is UNEVALUABLE for every component of this resolution** — the same outcome ACL-4 gives a missing enumerator, with the same publisher carve-out and the same reader refusal. Without that, a failed probe left the predicate undefined on the one input every ACE comparison depends on.
