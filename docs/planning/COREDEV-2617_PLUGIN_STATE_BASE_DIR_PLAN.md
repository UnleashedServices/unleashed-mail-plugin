...
| 47 | **drop bash's literal-glob `[ -e ]` guard** | an empty store yields zero entries **and no diagnostic** — the literal `base.*` must not reach rule 0, which would silently skip it and hide the missing guard |
