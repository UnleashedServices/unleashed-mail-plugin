...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2361`) and its matrix change (`:2367-2371`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2394-2401`) stand as written;
...
via `additionalContext` (§7 row `:2746`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2460`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2746`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2361`) requires that `paths.sh`'s absence change *who computes* the
...
| 152 | **accept a SECOND stat line** | an answer of two non-space lines followed by a well-formed ACE yields the store-level refusal above; under the mutation the second stat line is skipped as though the machine were still in INIT and the answer parses. Row 150 mutates the line-level skip and row 151 the missing initial state; neither covers a REPEATED one |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2746` amended.
...
_u13_answer_ok() {                              # stdin: the enumerator's answer
    ...
    while IFS= read -r _u13_l || [ -n "$_u13_l" ]; do
        case "$_u13_st" in
            INIT) case "$_u13_l" in
                      '')   return 1 ;;         # a blank FIRST line is not a stat line
                      ' '*) return 1 ;;         # an ACE before any stat line
                      *)    _u13_st=BODY ;;     # the ONE mandatory stat line
                  esac ;;
            BODY) case "$_u13_l" in
                      '')   : ;;                # BLANK
