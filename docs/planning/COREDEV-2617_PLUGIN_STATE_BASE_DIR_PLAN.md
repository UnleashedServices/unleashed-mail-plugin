...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2143`) and its matrix change (`:2149-2153`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2176-2183`) stand as written;
...
via `additionalContext` (§7 row `:2520`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2242`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2520`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2143`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2520` amended.
...
most one ACL walk per PROCESS **on the READ path**, because the protocol variables are set once and the five sourced A PUBLISHER walks more than once by construction and BUD-1 governs that count; this line is the
reader's and is not a cap on the publish path.
