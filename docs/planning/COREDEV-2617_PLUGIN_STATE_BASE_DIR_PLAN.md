...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2137`) and its matrix change (`:2143-2147`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2170-2177`) stand as written;
...
via `additionalContext` (§7 row `:2513`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2236`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2513`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2137`) requires that `paths.sh`'s absence change *who computes* the
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2513` amended.
...
**BUD-1 — Source-time budget: DERIVED and MEASURED, never asserted.** The resolution's cost is whatever PUB-7, PUB-9, SHARED-1 and the reader rules require, and **this rule states no numeric cap of its own**. What it requires is that the number be DERIVED from those rules' decision points and then MEASURED: §6's harness counts the actual `/usr/bin/uname -s`, `/bin/ls -lde` and `/usr/bin/getfacl -pc` invocations per resolution on each of the `created`, `current`, `conflict`, `stale`, `failed` and `none` paths, records them, and fails if a count exceeds what the rules derive — a discrepancy is a defect in the implementation or in the derivation, and the harness does not say which.
Two facts about the shape of that cost, because they are properties of the RULES rather than budget targets: a READER walks each chain once, since it authenticates each entry once and takes one exit. A PUBLISHER walks a chain more than once by construction, and the repeats are NOT redundant — each pair is separated by its own write or by another process's possible write, so **a verdict cached from before it is not evidence about the state after it**. On the no-write `current` path that is three authentications of the publisher's own entry: PUB-7's pre-write target-chain check, the write-or-skip predicate, and PUB-9's post-scan re-verification.
