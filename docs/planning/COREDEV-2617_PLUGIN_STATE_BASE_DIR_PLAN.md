...
> | restore consumed it | no `work-context-snapshot-*` remains, per the `rm -f "$SNAP"` at `sessionstart-restore.sh:108` |
>
...
**Consequences, all simplifications:** §4.3's round-6 mandate (`:2411`) and its matrix change (`:2417-2421`)
stand **exactly as written**; §4.4's quarantine premise and ordering (`:2444-2451`) stand as written;
...
via `additionalContext` (§7 row `:2796`), so a pointer naming attacker-writable storage is a
...
§5's inert-gate mitigation (`:2510`) is amended **in place**, not by reference — see the round-21 note
...
This **amends §7's consumer row** (`:2796`), which currently requires both snapshot scripts to leave
...
to kill, and §4.3's round-6 mandate (`:2411`) requires that `paths.sh`'s absence change *who computes* the
...
| 158 | **guard the `paths.sh` body on an inheritable flag** | `_UNLEASHED_PATHS_SH_LOADED=1` exported into a child that sources `paths.sh`: the specification defines its functions and resolves (`unleashed_plugin_base` prints the base or the sentinel); under the mutation the file defines NOTHING and `unleashed_plugin_base` is `command not found`. Both shells. |
...
* **`scripts/sessionstart-restore.sh`** — gains the one-line notice above; §7's row `:2796` amended.
